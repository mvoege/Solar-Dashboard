#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# index.py – Login-Schutz für JK-BMS Dashboard (Fix für Nginx & Real-IP)
...
if len(sys.argv) > 1:
    ...
    if cmd == "start":
        ...
        time.sleep(2)
        print(f"Gestartet -> http://solar.ddns.net:{PORT}")
        sys.exit(0)
    elif cmd == "stop":
...
class LoginHandler(http.server.BaseHTTPRequestHandler):
    ...
    def is_host_allowed(self):
        # Splittet Port ab, falls vorhanden (z.B. solar.ddns.net:1305 -> solar.ddns.net)
        host_header = self.headers.get('Host', '').lower().split(':')[0]
        client_ip = self.get_client_ip()
        
        allowed = ["solar.ddns.net", "localhost", "127.0.0.1"]
    ...
    def send_login_page(self, error=""):
        ...
        <style>body{{font-family:sans-serif;background:#000000;color:#fff;display:flex;justify-content:center;align-items:center;height:100vh;margin:0;}}
